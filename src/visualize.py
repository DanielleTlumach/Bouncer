...
def update_cache(staff: str, val: tuple[int, int], date: str):
    ...
    if not check_staff:
        ...
        if (bans + val[0] < 0) or (warns + val[1] < 0):
            print("Гей, у користувача буде негативний баланс, це недобре.")
...
def gen_monthly_plot():
    ...
    plt.ylabel("Журнали")
    plt.xlabel("Місяць")
    plt.title("Попередження/Бани за місяць")
