...
class MessageForwarder:
    ...
    def __init__(self, create_threads: bool, mailbox_channel: int, ban_appeal_channel: int, home_server: int,
        ...
        # Holds background tasks to keep them from being gc'd (https://docs.python.org/3/library/asyncio-task.html#asyncio.create_task)
    ...
    async def on_dm(self, message: discord.Message):
        ...
        try:
            ...
            await message.channel.send("Ваше повідомлення переслано!")
        except discord.errors.Forbidden as err:
            if err.code == 50007:
                await reply_channel.send(
                    "Неможливо відправити повідомлення вищевказаному користувачу - Неможливо надіслати повідомлення цьому користувачу")
            else:
                await reply_channel.send(
                    f"ПОМИЛКА: При спробі надіслати повідомлення про переадресацію виникла несподівана помилка. Повідом Danielle про це: {err}")
    ...
    async def _get_or_create_user_reply_thread(self, user: discord.User,
                                               parent_channel: discord.TextChannel) -> discord.Thread:
        """
        ...
        try:
            ...
            await parent_channel.send(
                f"Користувач <@{user.id}> надіслав викидайлу нове повідомлення (його відповіді не були заархівовані), посилання на гілку: <#{thread_id}>.")
            await self._update_reply_thread(user, user_reply_thread)
            return user_reply_thread
        except discord.errors.NotFound:
            # It was deleted
            await parent_channel.send(
                f"Гілку-відповідь для користувача <@{user.id}> не знайдено (ймовірно, її було видалено), створюю нову.")
            return await self._create_reply_thread(user, parent_channel)
    ...
    async def _create_reply_thread(self, user: discord.User, parent_channel: discord.TextChannel) -> discord.Thread:
        ...
        """
        thread = await parent_channel.create_thread(name=self._user_reply_thread_name(user),
    ...
    async def _add_staff_to_thread(self, thread: discord.Thread):
        ...
        """
        content = "Це згадка для додавання персоналу в цю гілку: "
...
class LRUCache:
    ...
    def set(self, result, *args):
        ...
        :param result: The result values.
