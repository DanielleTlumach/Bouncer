...
class UserLogEntry:
    ...
    def __str__(self):
        if self.log_type == LogTypes.BAN.value or self.log_type == LogTypes.SCAM.value:
            log_word = "Заблоковано"
        elif self.log_type == LogTypes.NOTE.value:
            log_word = "Примічено"
        elif self.log_type == LogTypes.KICK.value:
            log_word = "Вигнано"
        elif self.log_type == LogTypes.UNBAN.value:
            log_word = "Розблоковано"
        else:  # LogTypes.WARN
            log_word = f"Попереджено в {self.log_type}-ге"

        return f"[{format_time(self.timestamp)}] `{self.name}` - {log_word} членом персоналу {self.staff} - {self.log_message}\n"
...
def initialize():
    ...
    sqlconn.execute(
        "CREATE TABLE IF NOT EXISTS badeggs (dbid INT PRIMARY KEY, id INT, username TEXT, num INT, date DATE, message TEXT, staff TEXT, post INT);")
...
def add_log(log_entry: UserLogEntry):
    query = (
        "INSERT OR REPLACE INTO badeggs (dbid, id, username, num, date, message, staff, post) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        log_entry.as_list())
    _db_write(query)
...
def remove_log(dbid: int):
    query = (
        "REPLACE INTO badeggs (dbid, id, username, num, date, message, staff, post) VALUES (?, NULL, NULL, NULL, NULL, NULL, NULL, NULL)",
        [dbid])
    _db_write(query)
